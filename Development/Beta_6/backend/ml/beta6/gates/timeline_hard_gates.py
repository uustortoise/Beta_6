...
def evaluate_timeline_hard_gates(
    ...
    """
    require_timeline_metrics = bool(
        report.get("require_timeline_metrics")
        or (report.get("details") or {}).get("require_timeline_metrics")
    )
    raw_timeline_metrics = report.get("timeline_metrics")
    if raw_timeline_metrics is None:
        if require_timeline_metrics:
            return TimelineHardGateResult(
                checked=True,
                passed=False,
                reason_code=ReasonCode.FAIL_TIMELINE_METRICS_MISSING,
                details={"error": "timeline_metrics required but missing"},
            )

...
def _continuity_break_penalty(
    *,
    ...
    if prev_label == next_label:
        return 0.0
    if transition.continuity_break_penalty <= 0.0:
        return 0.0
    if transition.continuity_min_minutes <= 0.0:
        return 0.0
    continuity_labels = {str(label).strip().lower() for label in transition.continuity_labels}
    if not continuity_labels:
        return 0.0
    prev = str(prev_label).strip().lower()
    if prev not in continuity_labels:
        return 0.0

    prev_minutes = max(float(prev_run_steps), 1.0) * max(float(transition.step_minutes), 1e-6)
    min_minutes = max(float(transition.continuity_min_minutes), 1e-6)
    if prev_minutes >= min_minutes:
        return 0.0

    if float(observation_delta) >= float(transition.continuity_escape_margin):
        return 0.0

    under_ratio = (min_minutes - prev_minutes) / min_minutes
    return -float(transition.continuity_break_penalty) * float(under_ratio)
    ...
    for t in range(1, t_steps):
        for j in range(n_states):
            ...
            for i in range(n_states):
                ...
                continuity_penalty = _continuity_break_penalty(
                    prev_label=states[i],
                    next_label=states[j],
                    prev_run_steps=prev_run,
                    policy=policy,
                    observation_delta=float(observations[t, j] - observations[t, i]),
                )
                score = prev_score + transition[i, j] + penalty + continuity_penalty + observations[t, j]
                if score > best_score:
                    best_score = score
                    best_parent = i
                    best_run = next_run

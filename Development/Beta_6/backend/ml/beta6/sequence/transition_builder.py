...
class TransitionPolicy:
    ...
    continuity_labels: Tuple[str, ...] = ()
    continuity_min_minutes: float = 0.0
    continuity_break_penalty: float = 0.0
    continuity_escape_margin: float = 0.0
...
def load_duration_prior_policy(path: str | Path | None) -> DurationPriorPolicy:
    ...
    continuity_labels_raw = transition_cfg.get("continuity_labels", ())
    continuity_labels: Tuple[str, ...]
    if isinstance(continuity_labels_raw, str):
        continuity_labels = tuple(
            token
            for token in (_norm(item) for item in continuity_labels_raw.split(","))
            if token
        )
    elif isinstance(continuity_labels_raw, (list, tuple)):
        continuity_labels = tuple(
            token
            for token in (_norm(item) for item in continuity_labels_raw)
            if token
        )
    else:
        continuity_labels = ()
    transition = TransitionPolicy(
        ...
        continuity_min_minutes=max(0.0, float(transition_cfg.get("continuity_min_minutes", 0.0))),
        continuity_break_penalty=max(0.0, float(transition_cfg.get("continuity_break_penalty", 0.0))),
        continuity_escape_margin=max(0.0, float(transition_cfg.get("continuity_escape_margin", 0.0))),
    )

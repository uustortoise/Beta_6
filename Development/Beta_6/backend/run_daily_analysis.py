...
def _resolve_retrain_input_mode(default_mode: str | None = None) -> str:
    """
    ...
    if default_mode is None:
        ...
        if evidence_profile in {"pilot_stage_a", "pilot_stage_b"}:
            default_mode = "incoming_only"
        else:
            default_mode = "auto_aggregate"
...
def _resolve_release_gate_evidence_profile(default_profile: str = "production") -> str:
    ...
    if not raw and _is_beta6_authority_enabled():
        return "pilot_stage_a"
...
def _resolve_training_files_for_run(elder_id: str, incoming_files: list[Path]) -> tuple[str, list[Path], dict]:
    ...
    """
    mode = _resolve_retrain_input_mode()
...
def _snapshot_current_versions(registry: ModelRegistry, elder_id: str) -> dict[str, int]:
    ...
    for versions_file in model_dir.glob("*_versions.json"):
        ...
        try:
            current_version = int(registry.get_current_version(elder_id, room_name))
            if current_version <= 0:
                # Fallback: when no promoted champion exists yet, seed from latest saved
                # legacy version so RegistryV2 can still activate rollback fallback safely.
                all_versions = registry.list_model_versions(elder_id, room_name) or []
                current_version = max(
                    (int(v.get("version", 0)) for v in all_versions),
                    default=0,
                )
            versions[room_name] = int(max(0, current_version))
        except Exception:
    ...
    for metric in metrics:
        ...
        if not parity_checked:
            base_report = {
                ...
                "reason_code": ReasonCode.FAIL_RUNTIME_EVAL_PARITY.value,
                "details": {
                    "runtime_eval_parity_checked": False,
                    "runtime_eval_parity_passed": None,
                    "runtime_eval_parity_required": True,
                },
            }

...
def _authority_metric(**overrides):
    metric = {
        "room": "Bedroom",
        "gate_pass": True,
        "gate_reasons": [],
        ...
        "beta6_eval_policy": {"spike_suppression": True},
    }
...
def test_beta6_authority_blocks_room_when_parity_trace_absent(monkeypatch):
    monkeypatch.setattr("run_daily_analysis._is_beta6_authority_enabled", lambda: True)
    ...
    assert metrics[0]["beta6_room_decision"]["details"]["runtime_eval_parity_required"] is True
...
def test_phase62_pipeline_success_rate_tracks_execution_not_model_gate(monkeypatch):
    ...
    gate_pass, report = run_daily_analysis._apply_beta6_gate_authority(
        metrics=[_authority_metric(gate_pass=False, gate_reasons=["f1_below_floor"])],
        elder_id="HK001",
